...
export default class ContentfulModuleService {
    ...
    await this.managementClient.entry.update(
      ...
      },
      {
        ...
        fields: {
          ...productEntry.fields,
          productVariants: {
            [this.options.default_locale!]: product.variants?.map(
              (variant) => ({
                sys: {
                  type: "Link",
                  linkType: "Entry",
  ...
  async deleteProduct(productId: string) {
    try {
      ...
      for (const variant of productEntry.fields.productVariants[
        this.options.default_locale!
      ]) {
        await this.managementClient.entry.unpublish({
          environmentId: this.options.environment,
      ...
      for (const option of productEntry.fields.productOptions[
        this.options.default_locale!
      ]) {
        for (const value of option.fields.values[
          this.options.default_locale!
        ]) {
          await this.managementClient.entry.unpublish({
            environmentId: this.options.environment,
    ...
    return contentfulProducts.items.map((product) => {
      ...
      return {
        ...
        variants: (product.fields?.productVariants || []).map((variant) => {
          if (!variant?.fields) return null;
          
          // remove circular reference
          const {
            product: _,
            productOptionValues: __,
            ...variantFields
          } = variant.fields;
